...
class DataReader4npz:
    def __init__(self, data_name, data_dir):
        if data_name[:-4] in ('squirrel', 'crocodile', 'chameleon', 'squirrel_filtered', 'chameleon_filtered'):
            ...
            trn_idx = dataset['train_mask'][:,0]
            val_idx = dataset['val_mask'][:,0]
            tst_idx = dataset['test_mask'][:,0]
            trn_idx = np.where(trn_idx==1)[0]
            val_idx = np.where(val_idx==1)[0]
            tst_idx = np.where(tst_idx==1)[0]
            #     np.save('./traditional_split/crocodile_train_'+str(i)+'.npy',trn_idx)
            #     np.save('./traditional_split/crocodile_val_'+str(i)+'.npy',val_idx)
            #     np.save('./traditional_split/crocodile_tst_'+str(i)+'.npy',tst_idx)
            # exit(0)
            ...
            # ## 为deep walk采集数据
            # with open('crocodile.edgelist','w') as f:
            ...
            # torch.save(torch.from_numpy(labels),'crocodile_labels.pt')
            # exit(0)
            ...
            ## 为deep walk采集数据
            # with open('photo.edgelist','w') as f:
            #     for i in list(nx.to_edgelist(graph)):
            #         f.write(str(i[0])+'\t'+str(i[1])+'\r')
            # exit(0)
            # num_same_label = 0
            ...
            # print("Homophily ratio: {:.3f}".format(homophily_ratio))
            # exit(0)
            # torch.save(torch.from_numpy(labels),'photo_labels.pt')
            # exit(0)
